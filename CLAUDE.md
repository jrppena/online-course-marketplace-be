...
- **Prisma/Neon** owns the app profile (`role`, `bio`, …). [src/services/users.service.ts](src/services/users.service.ts)'s `getOrCreateProfile` auto-provisions a `User` row (role `STUDENT`) keyed by Firebase `uid` on first call — there is no signup endpoint. Name splitting (`displayName` → `firstName`/`lastName`) must stay in sync with the FE's MSW mock at `online-course-marketplace-fe/src/testing/mocks/handlers/users.ts`.
